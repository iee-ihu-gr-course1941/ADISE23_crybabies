...
BEGIN
	SELECT  p_color, p_num INTO piece_num, piece_color FROM `pawns` WHERE x=x1 AND y=y1;
	
	UPDATE `pawns` SET p_num=piece_num, p_color=piece_color WHERE X=x2 AND Y=y2;
	UPDATE `pawns` SET p_num=NULL, p_color=NULL WHERE X=x1 AND Y=y1;
