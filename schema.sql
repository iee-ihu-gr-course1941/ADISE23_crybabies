...
CREATE TABLE IF NOT EXISTS `board` (
  ...
  `b_fun` enum('R_sleep','B_sleep','G_sleep','Y_sleep','R_start','B_start','G_W','Y_start','W','S','R_finals','B_finals','G_finals','Y_finals','R_end','B_end','G_end','Y_end') DEFAULT NULL,
  PRIMARY KEY (`x`,`y`)
...
INSERT INTO `board_empty` (`x`, `y`, `b_color`, `b_fun`) VALUES
	(1, 1, 'B', NULL),
	(1, 2, 'B', NULL),
	(1, 3, 'B', NULL),
	(1, 4, 'B', NULL),
...
DELIMITER //
CREATE PROCEDURE `move_piece`(piece_color CHAR, piece_num CHAR, x1 TINYINT, y1 TINYINT, x2 TINYINT, y2 TINYINT)
BEGIN
	SELECT  p_color, p_num INTO piece_color, piece_num FROM `pawns` WHERE x=x1 AND y=y1;
	
	UPDATE `pawns` SET X=x2, Y=x2 WHERE X=x1 AND Y=y1 AND p_num=piece_num;
